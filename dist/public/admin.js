  ...
  tbody.innerHTML = keys.map(key => {
    ...
    return `
      <tr>
        ...
        <td>${usesText}</td>
        <td>
          <button class="table-action" onclick="copyKey('${key.key}')">Copy</button>
          ${key.is_active 
            ? `<button class="table-action revoke" onclick="revokeKey('${key.key}')">Revoke</button>`
            : `<button class="table-action" onclick="activateKey('${key.key}')">Activate</button>`
          }
          <button class="table-action revoke" onclick="deleteKey('${key.key}')">Delete</button>
        </td>
      </tr>
    `;
  }).join('');
