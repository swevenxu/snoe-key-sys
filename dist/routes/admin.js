...
import { createKey, createKeys, getKeyByValue, getAllKeys, revokeKey, activateKey, extendKey, deleteKey, getKeyLogs, getKeyStats, } from '../services/keyService.js';
 ...
 */
router.post('/keys', validateBody(schemas.createKey), async (req, res) => {
 ...
 */
router.post('/keys/batch', validateBody(schemas.createKeys), async (req, res) => {
 ...
 */
router.post('/keys/:key/activate', async (req, res) => {
 ...
 * Body: { days: number }
 */
router.post('/keys/:key/extend', validateBody(schemas.extendKey), async (req, res) => {
    const { days } = req.body;
 ...
 */
router.post('/keys/:key/reset-hwid', async (req, res) => {
    const key = await getKeyByValue(req.params.key);
    if (!key) {
        res.status(404).json({ error: 'Not Found', message: 'Key not found' });
        return;
 ...
 */
router.get('/keys/:key/logs', async (req, res) => {
