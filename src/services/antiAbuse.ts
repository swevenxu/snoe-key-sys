 ...
 */
export async function checkCanClaim(data: ClaimData): Promise<ClaimCheck> {
 ...
 */
export async function recordClaim(data: ClaimData & { keyId: number; keyValue: string }): Promise<void> {
  if (!config.antiAbuse.enabled) {
    return;
  }
 ...
 */
export async function getClaimStats(ipAddress: string): Promise<{
