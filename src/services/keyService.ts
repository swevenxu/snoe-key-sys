 ...
 */
export async function createKey(options: CreateKeyOptions = {}): Promise<Key> {
  const key = options.customKey ? normalizeKey(options.customKey) : generateKey();
  
  const result = await query<Key>(
    `INSERT INTO keys (key, note, duration_days, max_hwid_resets, max_uses)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
    [
      key,
 ...
 */
export async function createKeys(count: number, options: Omit<CreateKeyOptions, 'customKey'> = {}): Promise<Key[]> {
  const keys: Key[] = [];
 ...
 */
export async function getKeyByValue(keyValue: string): Promise<Key | null> {
  const result = await query<Key>(
 ...
 * Get a key by ID
 */
export async function getKeyById(id: number): Promise<Key | null> {
  const result = await query<Key>(
...
export async function getAllKeys(filters?: {
  ...
  const result = await query<Key>(sql, params);
  return result.rows;
}
...
export async function validateKey(
  ...
  await query(
    `UPDATE keys SET
       ...
       last_ip = $4
     WHERE id = $5`,
    [boundHwid, now, expiresAt, clientIp, key.id]
  );
 ...
 */
export async function resetHwid(keyValue: string): Promise<{ success: boolean; message: string }> {
  const key = await getKeyByValue(keyValue);
  ...
  await query(
    `UPDATE keys SET hwid = NULL, hwid_resets_used = hwid_resets_used + 1 WHERE id = $1`,
    [key.id]
  );
 ...
 */
export async function revokeKey(keyValue: string): Promise<boolean> {
 ...
 * Reactivate a key
 */
export async function activateKey(keyValue: string): Promise<boolean> {
  const result = await query(
    'UPDATE keys SET is_active = true WHERE key = $1',
    [normalizeKey(keyValue)]
  );
 ...
 */
export async function extendKey(keyValue: string, additionalDays: number): Promise<boolean> {
  const key = await getKeyByValue(keyValue);
 ...
 */
export async function deleteKey(keyValue: string): Promise<boolean> {
  const result = await query(
...
async function logValidation(
  ...
  try {
    await query(
      `INSERT INTO validation_logs (key_id, hwid, ip_address, success, failure_reason)
       VALUES ($1, $2, $3, $4, $5)`,
      [keyId, hwid || null, ip || null, success, failureReason]
    );
  } catch (error) {
 ...
 */
export async function getKeyLogs(keyValue: string, limit: number = 50): Promise<any[]> {
  const key = await getKeyByValue(keyValue);
  ...
  const result = await query(
    `SELECT * FROM validation_logs WHERE key_id = $1 ORDER BY created_at DESC LIMIT $2`,
    [key.id, limit]
  );
